...
hotspot_sp <- rast(file.path(data.dir, "spring_stopover_2500_v9_265_class.tif"))
hotspot_fa <- rast(file.path(data.dir, "fall_stopover_2500_v9_265_class.tif"))
## Read in counties shapefile
counties_shp <- st_read(file.path(data.dir, "counties.shp"))
cbsa_shp <- st_read(file.path(data.dir, "tl_2019_us_cbsa.shp"))
urbcore_shp <- st_read(file.path(data.dir, "tl_2023_us_uac20.shp"))
...
# write a function that takes hotspot raster, concerts to sf polygon
