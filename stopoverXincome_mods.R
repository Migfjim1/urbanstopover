...
# Only run this if interested in sensitivity analysis (Figure S1)
# Model is large (>300mb) and estimated run time is ~22 hours
#model_full <- brm(
...
# same here, only run for sensitivity analysis
#model_sub <- brm(
#  formula = mean_stopover_sp ~ prop.high + (1 + prop.high | Park_Urban),
...
# subsubset model (w >5 parks > than pixel size)
# this is model reported on in paper
model_subsub <- brm(
  formula = mean_stopover_sp ~ prop.high + (1 + prop.high | Park_Urban),
